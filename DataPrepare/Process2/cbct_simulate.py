...
class CBCTSimulator:
    def __init__(self):
        # 几何参数
        self.d_rows = 786  # 探测器行数  #
        self.d_cols = 1536  # 探测器列数  # 腹部为half-fan模式，未模拟
        self.d_space = 0.388 # 探测器像素尺寸 (mm) 0.388  0.392  0.776
        self.sdd = 1500
        self.sad = 1000
        self.angle_num = 300
        self.proj_angles = np.linspace(0, 2 * np.pi, self.angle_num, endpoint=False)  # 360个投影角度
        # 运动参数
        self.proj_group_num = 10  # 10
        self.gas_threshold = -160  # HU
        self.body_erosion = 6  # mm
        self.resp_amp_ap = 20  # mm
        self.resp_amp_lr = 0  # mm
        # 物理参数
        self.scatter_sigma = 5  # 值小锐利，值大模糊
        self.scatter_fraction = 0.15  # 0.3 ~ 0.6
        self.photon_flux = 5e7  # 入射光子通量
        self.beam_hard_a = 1  # 线性系数。 0.8 ~ 0.95，值越小硬化越明显
        self.beam_hard_b = -0.001 # 二次项系数，-0.1 ~ -0.01值越大，非线性越强
        self.min_proj_value = 0.01  # 最小截断，即探测器过饱和0.1
        self.max_proj_value = 10  # 截断
        # 物理效应
        ...
        self.direction = None
        # 控制参数
        self.u0 = 0.02  # 120KV下水的线性衰减系数，单位mm-1  # 0.19cm-1
        self.u_air = 0.0001
        # 存储
        # self.proj_geom = None
        # self.vol_geom = None
    ...
    def run(self, img:sitk.Image, mask):
        ...
        for group_idx in range(self.proj_group_num):
            # current_ct = moving_cts[group_idx]  # TODO to delete 用作测试
            current_ct = ct
    ...
    def _apply_simulate_effects(self, projections):
        ...
        # 散射效应 + 量子效应
        projections = self._apply_scatter_and_photon_effect(projections)
        # 探测器截断
        projections = self._apply_detector_trunc(projections)
        return projections

    def _apply_scatter_and_photon_effect(self, projections):
        # 投影值 -> 光子计数
        photon_flux = self.photon_flux * self.bowtie_filter_profile[np.newaxis, np.newaxis, :]
        photons = photon_flux * np.exp(-projections)
        # 光子数处理
        photons = self._apply_scatter_effect(photons)  # 散射效应导致光子数增加
        photons = np.random.poisson(photons)  # 加泊松噪声
        # 光子数 -> 投影值
        ratio = np.clip(photons / photon_flux, a_min=1e-8, a_max=1)
        projections = -np.log(ratio)
        return projections

    def _apply_scatter_effect(self, photons):
        # 低频散射
        scatter = gaussian_filter1d(photons, sigma=self.scatter_sigma, axis=0)
        scatter = gaussian_filter1d(scatter, sigma=self.scatter_sigma, axis=2)
        scatter *= self.scatter_fraction

        photons = photons + scatter
        return photons

    def _apply_beam_hardening(self, projections):
        """应用射线硬化效"""
        # projections = np.power(projections, self.beam_hard)
        v_max = np.max(projections)
        projections = self.beam_hard_a * projections + self.beam_hard_b * np.power(projections, 2)
        projections = np.clip(projections, 0, v_max)
        return projections
    ...
    def _simulate_gas_motion(self, hu: np.ndarray, u: np.ndarray, body_mask: np.ndarray):
        ...
        motion_simulator = BowelGasMotionSimulator(hu, u, body_mask,self.space[0], self.gas_threshold, self.body_erosion,
                                                   self.resp_amp_ap, self.resp_amp_lr)
        for group_idx in range(self.proj_group_num):
