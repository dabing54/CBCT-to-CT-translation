...
from cal_metrics_2d import brenner_gradient
...
def main():
    ...
    patient = '253142'   # 2PA047  -- 020.npy  032.npy   # 252505  252406
    item = '066.npy'   # 252456-35   253142-70-66  252930-21
    row_limit1 = 0 # 去除留白的行
    row_limit2 = 256
    ...
    series_name_list2 = ['DDIM-2step-360', 'DDIM-10step-360', 'DDIM-50step-360',
                         'DDIM-100step-360', 'DDIM-200step-360']
    ...
    manager.window.wm_geometry("+00+50")
    col = series_num
    # cbct
    ...
    plt.subplot(5, col, 5)
    brenner_base = brenner_gradient(ct1)
    brenner = brenner_gradient(sCT) / brenner_base
    plt.title('Unet zoom - %.3f' % brenner)
    plt.imshow(sCT, cmap='gray', vmin=display_min_val, vmax=display_max_val)
    plt.axis('off')
    ...
    for k, series_name in enumerate(series_name_list1):
        ...
        plt.title(series_name + '-%.3f' % brenner)  # predict
        plt.imshow(sCT, cmap='gray', vmin=display_min_val, vmax=display_max_val)
        plt.axis('off')
    # end for k
