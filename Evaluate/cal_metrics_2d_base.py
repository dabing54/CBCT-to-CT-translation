...
def main():
    ...
    out_path = r'E:\华为云盘\L1发展\进展项目\1-sCT\3-结果2\0-detal_result'
    series_name = 'dCBCT'  # sCBCT2

    out_path = os.path.join(out_path, series_name + '.csv')
    # ----------------------------
    patients = os.listdir(data_root_path_base)
    # patients = patients[:1]
    patients = patients[110:130]
    patient_num = len(patients)
    patient_out_list = list()
    for i, patient in enumerate(patients):
        print('Progress: %d/%d' % (i, patient_num))
        patient_path = os.path.join(data_root_path_base, patient)
        patient_ct_path = os.path.join(patient_path, 'CT')  # Full Dose Images
        patient_mask_path = os.path.join(patient_path, 'Mask')
        patient_cbct_path = os.path.join(patient_path, series_name)
        items = os.listdir(patient_ct_path)
        items = items[10:-10]  # 去除首尾多少层
        out_list = list()
        for item in items:
            ...
            # plt.subplot(2, 2, 4)
            # plt.imshow(gen-ct, vmin=-100, vmax=100)
            # plt.show()
