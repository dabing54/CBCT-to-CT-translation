...
class ResBlock(TimestepBlock):
    def __init__(self, channels:int, emb_channels,dropout,out_c=None, use_scale_shift_norm=False):
        super().__init__()
        self.channels = channels
        ...
        self.emb_layers = nn.Sequential(
            ...
            nn.Linear(
                emb_channels,
                2 * self.out_c if use_scale_shift_norm else self.out_c,
            ),
        )
...
class UNet(nn.Module):
    def __init__(self, in_c, out_c, model_c, channel_mult, num_res_blocks, attention_res, num_heads=1, dropout=0,
        ...
        time_embed_dim = model_c * 4
        self.time_embed = nn.Sequential(
            nn.Linear(model_c, time_embed_dim),
            nn.SiLU(),
    ...
    def forward(self, x, t, y):
        x = th.cat([x, y], dim=1)
        hs = []
        emb = self.time_embed(timestep_embedding(t, self.model_c))
